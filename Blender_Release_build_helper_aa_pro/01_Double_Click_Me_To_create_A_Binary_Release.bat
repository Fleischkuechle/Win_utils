...
set level1="%~dp0.."
set level2="%level1%\.."
set release_parent_folder_name="Auto_Release_binary"
...
REM now we compile all py filse in the releasedir to pbc binarys
call python -m compileall -b -f %RELEASE_DIR%

REM now we want to remove all py files from the release dir 
REM in exclude Py_delete_excluded_files.txt we define file we dont want to delete
set "excluded_files_file=Py_delete_excluded_files.txt"
REM in this for loop we go through all files with the ending .py and check if it is listed in the Py_delete_excluded_files.txt 
REM if not listed its getting deleted.
for /R "%RELEASE_DIR%" %%f in (*.py) do (
    set "exclude_file="

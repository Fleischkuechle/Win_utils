...
conda_env_path = os.path.join(parent_folder_path, "installer_files", "env")
...
def run_cmd(cmd, assert_success=False, environment=False, capture_output=False, env=None):
    ...
    if environment:
        if is_windows():
            conda_bat_path = os.path.join(parent_folder_path, "installer_files", "conda", "condabin", "conda.bat")
            cmd = f'"{conda_bat_path}" activate "{conda_env_path}" >nul && {cmd}'
        else:
            conda_sh_path = os.path.join(parent_folder_path, "installer_files", "conda", "etc", "profile.d", "conda.sh")
